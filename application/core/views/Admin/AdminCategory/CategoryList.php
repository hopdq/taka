...
<script type="text/javascript">
	function DeleteCategory(url, id) {
		var cf = confirm('Bạn có muốn xóa danh mục này ?');
		if (cf == true) {
			$(document).load(url, function(data) {
				if (data != "") {
					ShowAlert("Xóa thành công danh mục sản phẩm.", "success");
					$('#DtWrapper').html(data);
				}
				else {
					ShowAlert("Xảy ra lỗi trong quá trình xóa danh mục !!! Vui lòng thao tác lại !!!", "danger");
				}
			});
		} else {
			return 0;
	...
	//
	function AddCategoryPopup(url){
		$('#addedCategoryPopup').load(url, function(){
			var $registerForm = $("#addingCategoryForm").validate({
			...
			$('#addingCategoryForm').ajaxForm(function(data){
				if (data == -1) {
					ShowAlert("Xảy ra lỗi trong quá trình thêm danh mục ! Vui lòng thực hiện lại!", "danger");
				} else {
					$('#DtWrapper').html(data);
					ShowAlert("Đã thêm thành công danh mục mới !","success");
					$('#addedCategoryPopup').html("");
					$('#addingCategoryPage').html("");
				}
			});
		});
	}
...
<script type="text/javascript">
	...
	function EditCategory(url){
		$('#editedCategoryPopup').load(url, function(){
			var $registerForm = $("#editingCategoryForm").validate({
				// Rules for form validation
				rules : {
					categoryname : {
						required : true
			...
			$('#editingCategoryForm').ajaxForm(function(data){
				if (data == -1) {
					ShowAlert("Xảy ra lỗi trong quá trình sửa danh mục ! Vui lòng thực hiện lại!", "danger");
				} else {
					$('#DtWrapper').html(data);
					ShowAlert("Đã sửa thành công danh mục !", "success");
					$('#editedCategoryPopup').html("");
					$('#editingCategoryPage').html("");
				}
			});
		});
	}
